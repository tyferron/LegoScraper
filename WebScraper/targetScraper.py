...
target_url="https://www.target.com/p/lego-creator-3-in-1-deep-sea-creatures-shark-toy-set-31088/-/A-75559119#lnk=sametab"
...
try:
    title = soup.find('h1', {'data-test':'product-title'}).text.strip()
    setNum = title[-5:]
    title = title[5:len(title)-6]
...
try:
    #print(soup.find("div",{"class":"hrsYls"}))
    p = re.findall("\d+\.\d+", soup.find("span",{"class":"cobHST"}).text)
    o["price"]=float(p[0])
except:
    o["price"]=None
